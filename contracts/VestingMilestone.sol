...
contract VestingMilestone is Ownable, ReentrancyGuard {
    ...
    */
     ...
     */
    constructor(IERC20 _tokenAddress, address _allocation, address _recipient, address _owner, uint256 _intervalSecs, uint256 _period, bool _isDeposited) {
        ...
        numTokensReservedForVesting = _allocation
     ...
     */
    function _baseVestedAmount(uint40 _referenceTs)
        internal
        pure
        returns (uint256)
    {
        // If no schedule is created
        if (!startTime && deactivationTimestamp == 0) {
            return 0;
        }
        ...
        if (_referenceTs > .startTimestamp) {
            ...
            uint256 intervals = currentVestingDurationSecs / intervalSecs;
            ...
            // uint40 finalVestingDurationSecs = endTimestamp -
            //     startTimestamp; // length of the interval
