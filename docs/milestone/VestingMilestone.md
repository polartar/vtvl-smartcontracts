...
```solidity
function finalVestedAmount(uint256 _milestoneIndex) external view returns (uint256)
```
...
```solidity
function vestedAmount(uint256 _milestoneIndex, uint256 _referenceTs) external view returns (uint256)
```
